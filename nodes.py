...
class MCLTXVTiledVAEDecode(io.ComfyNode):
    """
    Tiled VAE Decode con overlap blending per immagini/video grandi.
    fork from ltxtricks
    """
    ...
    def define_schema(cls) -> io.Schema:
        return io.Schema(
            node_id="MCLTXVTiledVAEDecode",
            display_name="MC old LTXV Tiled VAE Decode",
            category="MC nodi/Ltxv",
            inputs=[
                io.Vae.Input("vae"),
                io.Latent.Input("latents"),
                io.Int.Input("horizontal_tiles", default=1, min=1, max=6),
                io.Int.Input("vertical_tiles", default=1, min=1, max=6),
                io.Int.Input("overlap", default=1, min=1, max=8),
                io.Boolean.Input("last_frame_fix", default=False),
            ],
            outputs=[
    ...
    def execute(
        ...
        if last_frame_fix:
            ...
            last_frame = samples[:, :, -1:, :, :]  # shape: [batch, channels, 1, height, width]
        ...
        for v in range(vertical_tiles):
            for h in range(horizontal_tiles):
                ...
                if output is None:
                    output = torch.zeros(
                        (batch, image_frames, output_height, output_width, decoded_tile.shape[-1]),
